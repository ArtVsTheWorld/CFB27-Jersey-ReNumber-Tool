...
export function shouldProcess(player) { if (player.IsNIL) return false; if (player.TeamIndex < 0 || player.TeamIndex === 255) return false; if (isPlaceholderPlayer(player)) return false; if (!VALID_POSITIONS.has(player.Position)) return false; return true; }