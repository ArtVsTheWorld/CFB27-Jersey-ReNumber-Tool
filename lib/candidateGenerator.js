...
function generateCandidates(player) {
    ...
    // Flatten preferred/fallback groups into simple arrays.
    const preferredCandidates = (rule.preferred ?? []).flat();
    const fallbackCandidates = (rule.fallback ?? []).flat();

    // If promotion attempt, only consider the primary preferred group.
    ...
    // Favored numbers heuristic — DTs prefer 90s, others prefer same digits.
    let favoredNumbers;
    const onesDigit = player.JerseyNum % 10, tensDigit = Math.floor(player.JerseyNum / 10);
