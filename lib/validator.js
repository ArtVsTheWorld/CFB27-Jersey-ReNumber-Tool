...
export function validateRoster(teamName, sideName, roster) {
    ...
    for (const [number, players] of grouped) { if (players.length > 1) { duplicates += players.length - 1; console.log(`\n${teamName} | ${sideName} | Duplicate #${number}`); for (const p of players) console.log(`   ${p.FirstName} ${p.LastName} (${p.Position}) (${p.SchoolYear}) | mustRenumber=${p.mustRenumberDuplicate}`); } }
